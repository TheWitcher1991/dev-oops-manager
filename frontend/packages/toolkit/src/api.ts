...
import { Dictionary, Paginated } from '@oops/types'

import { optimisticInvalidateQueries } from './react-query'
import { CrudRepository, ReadonlyRepository } from './repositories'
import { CreateRequestFx, createRequestFx } from './request'

export function createReadonlyApiFx<
	LIST_GET,
	GET,
	OPTIONS = Dictionary<any>,
	ID extends string | number = number,
>(fx: CreateRequestFx, config: { list: string; detail: string }) {
	const entitiesFx = (params?: Partial<OPTIONS>) =>
		fx<void, LIST_GET>({
			url: config.list,
			params,
		})

	const entityFx = (id: ID) =>
		fx<void, GET>({
			url: `${config.detail}/${id}/`,
		})

	return { entitiesFx, entityFx }
}
